...
class LBFloatNavigator extends StatefulWidget {

  @override
  _LBFloatNavigatorState createState() => _LBFloatNavigatorState();
} 


class _LBFloatNavigatorState extends State<LBFloatNavigator> 
                              with SingleTickerProviderStateMixin {

  int _activeIndex = 0; //激活项
  double _height = 48.0; //导航栏高度
  double _floatRadius = 0; //悬浮图标半径
  double _moveTween = 0.0; //移动补间
  double _padding = 10.0; //浮动图标与圆弧之间的间隙
  late AnimationController _animationController; //动画控制器
  late Animation<double> _moveAnimation; //移动动画
  List _navs = [
    Icons.search,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: Stack(children: [
        ...
        Positioned(
          bottom: 0.0,
          child: Container(color: Colors.white,height:40,width:width),
        ),
        Positioned(
          bottom: 40.0,
          child: Container(
...
class ArcPainter extends CustomPainter {
  ...
  ArcPainter({required this.navCount,required this.moveTween,required this.padding});
